...
## 简介
本指南面向希望将 PromptCocoPilot 集成到不同开发环境的工程师与使用者，覆盖 Claude Code、Qoder IDE 与 Codex 的集成方式。文档重点说明：
...
## 架构总览
整体架构遵循“轻量化重写器 + 结构化上下文 + 多宿主暴露”的设计，既可通过 MCP 工具在 Claude Code/Qoder 中使用，也可通过 Codex 本地 HTTP API 由外部按钮触发。
...
## 结论
PromptCocoPilot 提供统一的上下文感知提示词增强能力，通过 MCP 工具与 Skill 在 Claude Code 中无缝集成，通过 DevTools 与本地 HTTP API 在 Qoder 与 Codex 中扩展。建议优先使用真实模型增强，并结合结构化上下文与透明 before/after 展示，以获得最佳用户体验。
...
- Qoder（MCP + DevTools）
  - 优点：IDE 内集成、自动观察输入、稳定重连
  - 缺点：依赖 DevTools 端口与页面匹配
  - 适用：Quest/任务模式下的上下文增强
