...
图表来源
- [http_server.py:1-101](file://mcp-server/http_server.py#L1-L101)
...
## 架构总览
Codex 通过输入栏按钮触发本地 HTTP 请求，请求体包含草稿与上下文；HTTP 服务器解析请求后委托 MCP 工具完成增强；MCP 工具将结构化上下文打包并调用增强逻辑，最终返回增强后的提示词供用户审阅。
