...
## 更新摘要
**变更内容**
- 重大架构升级：从基于 Python 的 pywebview 系统完全迁移到 Swift 原生实现
- 新增全局热键支持：⌃⌥⌘P 全局热键触发增强界面
- 新增原生动画效果：基于 SwiftUI 的硬件加速动画
- 新增系统深度集成：原生窗口管理、刘海区域适配
- 新增原生构建系统：直接 swiftc 编译，无需 Xcode 项目
- Python 版本标记为过时但仍保持兼容性
...
**Swift 原生版本**：全新的 Swift 原生界面提供了完整的 macOS 原生体验，包括动态岛屿风格、全局热键支持、原生动画效果和系统深度集成。
