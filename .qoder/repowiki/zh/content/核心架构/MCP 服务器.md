...
## 更新摘要
**变更内容**
- 更新默认模型偏好：从 deepseek-v4-flash 更改为 qwen-flash
- 强调 qwen-flash 的中文文本重写优势
- 更新 MCP 服务器配置与部署说明中的模型相关信息
...
- mcp-server：MCP 服务器与核心增强逻辑
  ...
  - enhance.py：核心增强逻辑，支持真实 Dashscope 调用，默认使用 qwen-flash 模型
...
E["enhance.py<br/>核心增强逻辑<br/>默认模型: qwen-flash"]
...
- 增强逻辑：基于 Dashscope 的真实 LLM 调用，默认使用 qwen-flash 模型，支持严格系统指令与清理输出
- 上下文打包：将对话、代码事实、任务状态、编辑器上下文等结构化为增强器输入
- 本地 HTTP API：为 Codex 按钮提供稳定端点，返回优化后的输入文本
- 技能定义：指导 Claude Code 在何时、如何调用工具并呈现优化前后对比

**章节来源**
- [mcp-server/server.py:42-261](file://mcp-server/server.py#L42-L261)
- [mcp-server/enhance.py:22-175](file://mcp-server/enhance.py#L22-L175)
...
participant Enhance as "增强逻辑(enhance.py)<br/>默认模型 : qwen-flash"
...
**图表来源**
- [mcp-server/server.py:82-261](file://mcp-server/server.py#L82-L261)
...
Pack --> Enhance["调用增强逻辑<br/>默认模型: qwen-flash"]
UseCtx --> Enhance
...
- 实际调用（Dashscope）
  ...
  - 默认模型：qwen-flash（专为快速中文文本重写设计）
  - 可通过环境变量 ENHANCE_MODEL 指定其他模型
...
**更新** 默认模型已从 deepseek-v4-flash 更新为 qwen-flash，提供更快的中文文本重写能力
...
- 与 Dashscope 集成
  - 通过环境变量自动加载 API Key
  - 默认模型：qwen-flash（可通过 ENHANCE_MODEL 覆盖）
...
- 最佳实践
  ...
  - 默认使用 qwen-flash 模型，提供更快的中文文本重写能力
...
```mermaid
graph LR
Server["server.py"] --> Enhance["enhance.py<br/>默认模型: qwen-flash"]
...
- 模型选择
  - 默认使用 fast 模型（qwen-flash），提供更快的中文文本重写能力
...
## 结论
本 MCP 服务器以轻量、可移植的方式实现了上下文感知的提示词增强能力，通过 JSON-RPC stdio 协议与 Claude Code、Qoder 等客户端无缝对接。结合技能定义与结构化上下文，能够显著提升用户输入的质量与可执行性。通过 Dashscope 真实调用与回退策略，兼顾效果与稳定性；通过上下文预算与 qwen-flash 模型，保障性能与用户体验。默认使用 qwen-flash 模型提供更快的中文文本重写能力，建议在生产环境中配置 API Key 并启用结构化上下文传递，以获得最佳增强效果。
...
### MCP 工具接口实现参考路径
- 工具注册与参数定义：[mcp-server/server.py:108-229](file://mcp-server/server.py#L108-L229)
