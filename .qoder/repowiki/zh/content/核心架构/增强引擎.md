...
**更新** 增强引擎现已完全迁移到中文输出，提供更符合中文编程环境和用户习惯的增强体验，特别针对Kilo Code集成缺口进行了优化。
...
- 增强核心（enhance.py）
  ...
  - 增强函数：支持字符串上下文与生成函数回调；内置回退策略；生产环境默认通过 Dashscope 实时调用。
...
- 生成策略：
  ...
  - 生产实调：优先加载 Dashscope API Key，调用兼容 OpenAI 接口，设置温度、最大令牌等参数，解析响应并清洗输出。
...
HasKey --> |是| RealCall["发起 Dashscope 实时调用"]
...
- 模型选择：默认模型可通过环境变量配置；在 MCP 服务器中自动加载环境变量或特定路径下的 .env 文件中的 API Key。
...
**更新** 中文指令重构显著提升了与中文编程环境的契合度，特别是针对Kilo Code集成缺口的解决，为用户提供更加自然和高效的增强体验。
...
C --> D{"是否提供生成函数?"}
D --> |是| E["调用生成函数"]
D --> |否| F["Dashscope 实时调用"]
E --> G["清洗输出"]
F --> G
G --> H["返回增强文本供审阅"]
```

**更新** 中文指令重构后的流程图体现了更符合中文用户习惯的增强体验，特别是在结构化格式和可执行性方面的强化。