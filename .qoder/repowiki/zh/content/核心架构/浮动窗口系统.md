...
- **更新** 从 borderless window 架构迁移到基于 NSPanel 的 KeyablePanel 实现，解决焦点冲突问题
...
**更新** 系统现已采用全新的 Swift 原生架构，完全替代了原有的 pywebview 实现，提供了更流畅、更稳定的 macOS 原生体验。**更新** 从 borderless window 架构迁移到基于 NSPanel 的 KeyablePanel 实现，彻底解决了焦点冲突问题，并支持刘海屏停靠。
...
**图表来源**
- [App.swift:1-480](file://claude-ui/swift/Sources/App.swift#L1-L480)
...
**更新** 全新 Swift 原生架构，基于 NSPanel 的 KeyablePanel 实现

1. **PromptCocoIsland (Swift 原生)**: 基于 SwiftUI 的原生 macOS 应用，支持刘海屏停靠和全局热键
2. **KeyablePanel**: 基于 NSPanel 的可按键面板，解决焦点冲突问题
3. **NotchHostingView**: 专为刘海屏优化的托管视图
4. **EnhanceClient**: Swift 原生增强客户端，与现有 MCP 服务器完全兼容
5. **SessionReader**: Swift 原生会话读取器，直接访问 Claude Code 数据存储
6. **Selection**: Swift 原生剪贴板操作，支持无障碍权限合成键盘事件
...
| 性能表现 | ✅ 更流畅稳定 | ✅ 足够稳定 |
| 系统集成 | ✅ 深度集成 | ✅ 基础集成 |
| 焦点管理 | ✅ KeyablePanel 解决冲突 | ❌ 传统窗口焦点问题 |

**更新** Swift 原生实现提供了更优秀的性能和系统集成体验，特别是 KeyablePanel 解决了焦点冲突问题

**章节来源**
- [App.swift:9-66](file://claude-ui/swift/Sources/App.swift#L9-L66)
- [App.swift:226-274](file://claude-ui/swift/Sources/App.swift#L226-L274)
...
KP[KeyablePanel<br/>可按键面板]
NHV[NotchHostingView<br/>刘海屏托管视图]
...
**更新** 新增 KeyablePanel 和 NotchHostingView 组件，解决焦点冲突问题
...
class KeyablePanel {
+canBecomeKey : Bool
+override canBecomeKey() Bool
}
class NotchHostingView {
...
IslandWindowController --> KeyablePanel : "使用"
IslandWindowController --> AppState : "使用"
KeyablePanel --> NotchHostingView : "包含"
```

**图表来源**
- [App.swift:8-480](file://claude-ui/swift/Sources/App.swift#L8-L480)
...
participant KP as KeyablePanel
...
**更新** 全新的 KeyablePanel 实现，解决焦点冲突问题

KeyablePanel 是基于 NSPanel 的自定义面板类，专门设计来解决传统窗口的焦点冲突问题：
...
非激活状态 --> 激活状态 : 用户点击
激活状态 --> 非激活状态 : 失去焦点
非激活状态 --> 激活状态 : KeyablePanel.makeKey()
激活状态 --> 激活状态 : 用户输入
```

**图表来源**
- [App.swift:232-234](file://claude-ui/swift/Sources/App.swift#L232-L234)

#### KeyablePanel 特性

| 特性 | 描述 | 实现方式 |
|------|------|----------|
| 可按键 | ✅ canBecomeKey = true | 重写 canBecomeKey 属性 |
| 非激活 | ✅ .nonactivatingPanel | 使用 NSPanel 样式掩码 |
| 无阴影 | ✅ hasShadow = false | 禁用阴影效果 |
| 透明背景 | ✅ backgroundColor = .clear | 设置透明背景 |
| 浮动面板 | ✅ isFloatingPanel = true | 确保面板浮动 |
| 窗口层级 | ✅ Level(mainMenuWindow)+2 | 置于菜单栏之上 |
| 集合行为 | ✅ canJoinAllSpaces | 支持多空间显示 |

**章节来源**
- [App.swift:226-234](file://claude-ui/swift/Sources/App.swift#L226-L234)

### NotchHostingView (刘海屏优化)

**更新** 全新的 NotchHostingView 实现

NotchHostingView 是专为刘海屏优化的托管视图，解决 SwiftUI 在刘海屏区域的显示问题：

#### 刘海屏融合机制

```mermaid
flowchart TD
A[SwiftUI 视图] --> B[NotchHostingView]
B --> C{安全区域检测}
C --> |有刘海屏| D[safeAreaInsets = 0]
C --> |无刘海屏| E[使用默认安全区域]
D --> F[黑色背景绘制到刘海屏]
E --> G[保留默认刘海屏区域]
F --> H[无缝融合效果]
...
**图表来源**
- [App.swift:243-274](file://claude-ui/swift/Sources/App.swift#L243-L274)

#### NotchHostingView 特性

| 特性 | 描述 | 实现方式 |
|------|------|----------|
| 安全区域 | ✅ safeAreaInsets = 0 | 重写安全区域属性 |
| 首次点击 | ✅ acceptsFirstMouse = true | 允许首次鼠标点击 |
| 点击处理 | ✅ mouseDown → makeKey | 点击时使面板获得焦点 |
| 延迟布局 | ✅ deferred layout | 避免显示循环重入崩溃 |
| 崩溃防护 | ✅ needsUpdateConstraints | 防止约束更新崩溃 |
...
**章节来源**
- [IslandView.swift:21-395](file://claude-ui/swift/Sources/IslandView.swift#L21-L395)
...
2. **KeyablePanel 依赖**: KeyablePanel 依赖 Cocoa AppKit 的 NSPanel
3. **NotchHostingView 依赖**: NotchHostingView 依赖 NSHostingView
4. **会话读取依赖**: SessionReader 直接依赖文件系统 API
5. **增强服务依赖**: EnhanceClient 依赖网络请求和环境变量
6. **剪贴板依赖**: Selection 依赖无障碍权限和系统事件
7. **传统组件依赖**: Python 组件依赖 urllib.error 进行异常处理
8. **外部 API 依赖**: 增强逻辑依赖 Dashscope API 进行实际的文本重写

**更新** 新增 KeyablePanel 和 NotchHostingView 的依赖关系
...
5. **KeyablePanel 优化**: 解决焦点冲突，避免不必要的窗口激活
...
5. **延迟布局**: NotchHostingView 的延迟布局机制防止崩溃
...
6. **KeyablePanel**: 解决焦点冲突，提供更好的用户体验
...
**可能原因**:
1. KeyablePanel 配置不正确
2. NotchHostingView 安全区域设置问题
3. 窗口样式掩码配置错误

**解决方法**:
```bash
# 检查 KeyablePanel 配置
# 确保 canBecomeKey = true
# 确保使用 .nonactivatingPanel 样式

# 检查 NotchHostingView 配置
# 确保 safeAreaInsets = 0
# 确保 acceptsFirstMouse = true
```
...
6. **焦点管理优化**: KeyablePanel 解决了传统窗口的焦点冲突问题
7. **向后兼容性**: 保持与现有 Python 组件和 MCP 服务器的完全兼容
8. **现代化界面**: 基于 SwiftUI 的现代设计语言，支持流畅动画和响应式布局
9. **系统级集成**: 深度集成 macOS 系统 API，提供原生的应用体验

**更新** 新版本显著提升了性能、稳定性和用户体验，特别是 KeyablePanel 的引入彻底解决了焦点冲突问题，同时保持了与现有生态系统的兼容性
